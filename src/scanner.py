    ...
    def __init__(self, source: str, lox_main):
        ...
        self.__token_strings: dict[str, callable] = \
            {"(": lambda c: TokenType.LEFT_PAREN,
             ...
             "^": lambda c: TokenType.CARET,
             "!": lambda c: TokenType.BANG_EQUAL if self.__match("=") else TokenType.BANG,
             "=": lambda c: TokenType.EQUAL_EQUAL if self.__match("=") else TokenType.EQUAL,
             "<": lambda c: TokenType.LESS_EQUAL if self.__match("=") else TokenType.LESS,
             ">": lambda c: TokenType.GREATER_EQUAL if self.__match("=") else TokenType.GREATER,
             "/": lambda c: self.__slash_handler(),
             " ": lambda c: None,
             "\t": lambda c: None,
             "\r": lambda c: None,
             "\n": lambda c: self.__newline_handler(),
             "\"": lambda c: self.__string()
             }
